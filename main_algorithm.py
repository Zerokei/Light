...
def init_data(config, encoding=None):
    # 读取指定发电输出曲线文件
    df = None
    encodings = [encoding, get_file_encoding(config['outputCurvePath']), "utf-8", "cp1252","gbk", "gb2312", "gb18030", "utf-16", "big5","ascii","latin1", "latin-1","unicode"]
...
def calc_realtime_power_data(power_curve):
    ...
    realtime_a = np.full(len(power_curve), '-')  # 实时电流
    realtime_power = power_curve  # 实时功率
    energy_curve = np.cumsum(power_curve) / 3600  # 累计发电量
    realtime_energy = np.array([energy_curve[i] if i < 3600 else energy_curve[i] - energy_curve[i - 3600] for i in
                       range(len(energy_curve))])  # 实时发电量（当前一小时发电量）
    realtime = np.array([str(i // 3600).zfill(2) + ':' + str((i % 3600) // 60).zfill(2) + ':' + str(i % 60).zfill(2) for i in
        range(86400)])
